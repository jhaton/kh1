...
def write_command_symbol_manifest(
    ...
    if output_format == "splat":
        path.write_text(
            "".join(
                f"{symbol['symbol']:<72} = {symbol['address']}; // type:func\n"
                for symbol in manifest["symbols"]
            )
        )
        return
...
def _command_symbol_renames(manifest: dict[str, object]) -> dict[str, str]:
    symbols = manifest.get("symbols")
    if not isinstance(symbols, list):
        raise FormatError("command symbol manifest has no symbols array")
    renames: dict[str, str] = {}
    assigned_names: set[str] = set()
    for entry in symbols:
        if not isinstance(entry, dict):
            raise FormatError("command symbol manifest entry is not an object")
        current_symbol = entry.get("current_symbol")
        symbol = entry.get("symbol")
        if not isinstance(current_symbol, str) or not isinstance(symbol, str):
            raise FormatError("command symbol manifest entry lacks symbol names")
        if current_symbol == symbol:
            continue
        if re.fullmatch(r"func_[0-9A-Fa-f]{8}", current_symbol) is None:
            raise FormatError(f"refusing to replace non-autogenerated symbol {current_symbol}")
        if re.fullmatch(r"ScriptCommand_[A-Za-z0-9_]+", symbol) is None:
            raise FormatError(f"invalid generated command symbol {symbol}")
        if current_symbol in renames:
            raise FormatError(f"duplicate source symbol in manifest: {current_symbol}")
        if symbol in assigned_names:
            raise FormatError(f"duplicate target symbol in manifest: {symbol}")
        renames[current_symbol] = symbol
        assigned_names.add(symbol)
    return renames


def _symbol_source_files(paths: Iterable[Path]) -> tuple[Path, ...]:
    allowed_suffixes = frozenset(
        (".c", ".h", ".inc", ".json", ".ld", ".md", ".py", ".s", ".txt", ".yaml", ".yml")
    )
    files: set[Path] = set()
    for path in paths:
        if path.is_file():
            if path.suffix.lower() not in allowed_suffixes:
                raise FormatError(f"unsupported symbol propagation file: {path}")
            files.add(path)
            continue
        if not path.is_dir():
            raise FormatError(f"symbol propagation path does not exist: {path}")
        files.update(
            candidate
...
def propagate_command_symbols(
    manifest: dict[str, object], paths: Iterable[Path], apply: bool = False
) -> dict[str, object]:
    renames = _command_symbol_renames(manifest)
    if not renames:
        return {
            "symbols": 0,
            "files_scanned": 0,
    ...
    files = _symbol_source_files(paths)
    identifier_characters = "A-Za-z0-9_.$"
    old_pattern = re.compile(
        rf"(?<![{identifier_characters}])"
        rf"({'|'.join(re.escape(symbol) for symbol in sorted(renames, key=len, reverse=True))})"
        rf"(?![{identifier_characters}])"
    )
    new_pattern = re.compile(
        rf"(?<![{identifier_characters}])"
        rf"({'|'.join(re.escape(symbol) for symbol in sorted(renames.values(), key=len, reverse=True))})"
        rf"(?![{identifier_characters}])"
    )
    ...
    for path in files:
        text = path.read_text()
        contents[path] = text
        old_counts.update(match.group(1) for match in old_pattern.finditer(text))
        new_counts.update(match.group(1) for match in new_pattern.finditer(text))

    missing = sorted(set(renames) - set(old_counts))
    if missing:
        raise FormatError(f"manifest symbols have no repository references: {missing}")
    partial = sorted(symbol for symbol in renames.values() if new_counts[symbol])
    if partial:
        raise FormatError(f"target symbols already exist before propagation: {partial}")

    changed: dict[Path, str] = {}
    replacement_count = 0
    for path, text in contents.items():
        rewritten, count = old_pattern.subn(
            lambda match: renames[match.group(1)],
            text,
        )
        if count:
...
def _propagate_symbols(args: argparse.Namespace) -> int:
    manifest = json.loads(args.manifest.read_text())
    result = propagate_command_symbols(manifest, args.path, args.apply)
...
def main() -> int:
    ...
    export_symbols.add_argument(
        "--format", choices=("json", "pcsx2", "splat"), default="json"
    )
    export_symbols.set_defaults(action=_export_symbols)

    propagate_symbols = subparsers.add_parser(
        "propagate-symbols",
        help="replace autogenerated command handler tokens from a symbol manifest",
    )
    propagate_symbols.add_argument("manifest", type=Path)
    propagate_symbols.add_argument("--path", type=Path, action="append", required=True)
    propagate_symbols.add_argument("--apply", action="store_true")
    propagate_symbols.set_defaults(action=_propagate_symbols)
